...
Updated for VitalChronicle **1.1.15**.
...
1. The release-preparation script keeps a manually advanced semantic version such as
   `1.1.15`, or advances the patch version when the source still matches the latest tag.
...
as `v1.1.15`. Existing release assets are replaced by filename.
