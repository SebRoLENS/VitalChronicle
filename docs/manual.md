...
date: "Updated for VitalChronicle 1.2.0"
lang: en-US
...
This is the authoritative user manual for **VitalChronicle 1.2.0**. VitalChronicle is a
...
2. Make it executable:
   ...
   ```bash
   chmod +x VitalChronicle-1.2.0-linux-x86_64.AppImage
   ```

3. Start it:

   ```bash
   ./VitalChronicle-1.2.0-linux-x86_64.AppImage
   ```

If FUSE is unavailable, run it with:

```bash
APPIMAGE_EXTRACT_AND_RUN=1 ./VitalChronicle-1.2.0-linux-x86_64.AppImage
```
...
1. Download `VitalChronicle-1.2.0-windows-x86_64.exe`.
...
existing 0.2.12 archives survive the 1.2.0 upgrade. Typical locations are:
...
VitalChronicle 1.2.0 does not ship an APK. The current application uses desktop Qt widgets,
